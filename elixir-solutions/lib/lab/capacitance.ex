...
defmodule Capacitance do
  ...
  defp print_commercial_cap(%{
    ...
    return "#{name}|#{stated}|#{measured}|#{tolerance * 100}|#{Float.round(reality * 100, 2)}|#{reality <= tolerance}|"
  end
  ...
  def part_two() do
    ...
    IO.puts("Position | Height (m) | δheight (m) | %δheight | Width (m) | δwidth (m) | %δwidth | Capacitance (nF)")
    IO.puts("-| -| -| -| -| -| -| -|")
    res
  ...
  defp print_variance_area1(data) do
    return "#{data.pos} A4|#{Float.round(data.h * 0.001, 3)}|#{Float.round(data.u_h * 0.001, 3)}|#{Float.round(data.p_h * 100, 2)} %|#{Float.round(data.w * 0.001, 3)}|#{Float.round(data.u_w * 0.001, 3)}|#{Float.round(data.p_w * 100, 2)}|#{data.c}"
  end
  ...
  defp print_second(data) do
    return "#{data.pos} A4|#{Float.round(data.area * 0.000001, 4)}|#{Float.round(data.u_area * 0.000001, 4)}|#{Float.round(data.p_area * 100, 2)}|#{data.c}|#{data.u_c}"
  end
  ...
  def part_three() do
    ...
    IO.puts("# of pages | Plate Separation d (m) | 1/d (m-1) | Measurement 1 | Measurement 2 | C_ave (nF) | ± C_ave (nF")
    IO.puts("-| -| -| -| -| -| -|")
    [
      # of sheets C1 C2
