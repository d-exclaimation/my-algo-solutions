...
defmodule AbsoluteZero do
  ...
  def only_part() do
    ...
    # average of both
    %{reg: with_avg, uncertain: with_avr} = res_with |> Enum.map(fn {_, res} -> res end) |> average_temperature()
    %{reg: without_avg, uncertain: without_avr} = res_without |> Enum.map(fn {_, res} -> res end) |> average_temperature()
    IO.puts("With liquid nitrogen avg T = #{with_avg} | #{with_avr}, and without avg T = #{without_avg} | #{without_avr}")
  end
  ...
  def pairs_apply(arr, something) do
    ...
    |> Enum.map(fn pair ->
        ...
        return {"#{first + 1}, #{second + 1}", something.(Enum.at(arr, first), Enum.at(arr, second))}
      end)
  end
